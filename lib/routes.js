...
var wrapResponse = require('./wrap_response.js')
var jsonToXml = require('./json_to_xml.js')

var getMap = require('../actions/get_map.js')
...
route('GET /api/(0.6)?/capabilities', function (req, res, osm) {
  res.end(wrapResponse(null, capabilities))
})

route('GET /api/0.6/map?*', function (req, res, osm) {
  // TODO: Validate bbox
  ...
  pump(r, jsonToXml, wrapResponse(bbox), res, function (err) {
    if (err) return error(500, res, err)
  })
})

route('PUT /api/0.6/:type(node|way|relation|changeset)/create', function (req, res, osm, m) {
...
route('GET /api/0.6/changeset/:id/download', function (req, res, osm, m) {
...
route('GET /api/0.6/:type(node|way|relation)/:id/history', function (req, res, osm, m) {
  res.setHeader('content-type', 'text/xml')
...
route('GET /api/0.6/:type(node|way|relation)/:id/:version', function (req, res, osm, m) {
...
route('GET /api/0.6/:type(node|way|relation)\\?:param=:ids', function (req, res, osm, m) {
...
route('GET /api/0.6/:type(node|way|relation)/:id', function (req, res, osm, m) {
...
route('GET /api/0.6/:type(nodes|ways|relations)\\?:ktype=:ids', function (req, res, osm, m) {
...
route('POST /api/0.6/changeset/:id/upload', function (req, res, osm, m) {
  var results = []
  ...
  function onbody (body) {
    ...
    del(osm, ops.delete, function (err, xbatch, xresults) {
      if (err) {
        error(err.code, res, err.message)
      } else {
...
route('PUT /api/0.6/changeset/:id/close', function (req, res, osm, m) {
...
route('DELETE /api/0.6/:type(node|way|relation)/:id', function (req, res, osm, m) {
  req.pipe(concat({ encoding: 'string' }, onbody))
  function onbody (body) {
    var ops = xmlcreate(body)
    if (ops.length !== 1) {
      return error(400, res, 'Only one element can be deleted per request.')
    }
    if (ops[0].type !== m.params.type) {
      return error(400, res, 'Type mismatch between url parameter and xml.')
    }
    if (ops[0].id !== m.params.id) {
      return error(400, res, 'id mismatch between url parameter and xml.')
    }
