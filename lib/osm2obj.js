...
module.exports = function (api, opts) {
  ...
  var out = through.obj(function (doc, _, cb) {
    ...
    api.getElement(null, {version: doc.version}, function (err, old) {
      ...
      // If there's an existing element that's an observation, Use that
      // observation, and then patch the changes from iD editor to it. This is
      // necessary because incoming OSM changesets are not required to maintain
      // these non-OSM Mapeo attributes through the entire lifecycle, so we
      // patch these from the back end. Lat & Lon are ignored, because
      // observation location should be immutable. We have not yet implemented a
      // way in iD Editor to prevent moving observations, so we ignore any
      // location edits on save
